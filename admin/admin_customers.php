...
if (isset($_GET['search'])) {
    ...
    $where = "(full_name LIKE '%$search%' OR customer_email LIKE '%$search%' OR phone LIKE '%$search%' OR address LIKE '%$search%')";
}
...
if (isset($_GET['action'])) {
    ...
    switch ($_GET['action']) {
        ...
        case 'deactivate':
            $update_query = "UPDATE customer SET status = 'inactive' WHERE customer_id = $customer_id";
            mysqli_query($conn, $update_query);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['bulk_action'])) {
    if (isset($_POST['selected_customers']) && is_array($_POST['selected_customers'])) {
        ...
        switch ($_POST['bulk_action']) {
            case 'activate':
                $update_query = "UPDATE customer SET status = 'active' WHERE customer_id IN ($ids_string)";
                mysqli_query($conn, $update_query);
                $_SESSION['success'] = count($selected_ids) . " customer(s) activated successfully!";
                break;
                
            case 'deactivate':
                $update_query = "UPDATE customer SET status = 'inactive' WHERE customer_id IN ($ids_string)";
                mysqli_query($conn, $update_query);
                $_SESSION['success'] = count($selected_ids) . " customer(s) deactivated successfully!";
                break;
                
            case 'delete':
                $delete_query = "DELETE FROM customer WHERE customer_id IN ($ids_string)";
                mysqli_query($conn, $delete_query);
                $_SESSION['success'] = count($selected_ids) . " customer(s) deleted successfully!";
                break;
        }
    }
...
// Get statistics for sidebar and cards
$stats = [];
$queries = [
    'total_customers' => "SELECT COUNT(*) as count FROM customer",
    'active_customers' => "SELECT COUNT(*) as count FROM customer WHERE status = 'active'",
    'total_points' => "SELECT SUM(points) as count FROM customer",
...
<head>
    ...
    <title>Customer Management - WasteWise</title>
    <!-- Tailwind CSS -->
    ...
    <style>
        * {
        ...
        .status-active { background: rgba(220, 252, 231, 0.2); color: #dcfce7; border: 1px solid rgba(220, 252, 231, 0.3); }
        .status-inactive { background: rgba(254, 226, 226, 0.2); color: #fee2e2; border: 1px solid rgba(254, 226, 226, 0.3); }
        ...
        .btn-activate { background: rgba(34, 197, 94, 0.2); color: #22c55e; border: 1px solid rgba(34, 197, 94, 0.3); }
        .btn-activate:hover { background: rgba(34, 197, 94, 0.3); }
        .btn-deactivate { background: rgba(245, 158, 11, 0.2); color: #f59e0b; border: 1px solid rgba(245, 158, 11, 0.3); }
        .btn-deactivate:hover { background: rgba(245, 158, 11, 0.3); }
    ...
    <main class="main-content">
        ...
        <nav class="flex justify-between items-center py-4 md:py-6 px-4 md:px-6">
            ...
            <div class="hidden md:block">
                <h1 class="text-white text-2xl font-bold">Customer Management</h1>
                <p class="text-gray-400 text-sm">Manage customer accounts and track their contributions</p>
            </div>
        ...
        <div class="p-4 md:p-6">
            ...
            <div class="mb-8 md:mb-12 stagger-animation">
                ...
                <p class="text-gray-200 text-base sm:text-lg md:text-xl mb-6 md:mb-8 leading-relaxed">
                    Manage customer accounts, track contributions, and view detailed analytics.
                </p>
                    ...
                    <form method="GET" action="">
                        <div class="flex flex-col md:flex-row gap-4">
                            ...
                            <?php if($search || (isset($_GET['status']) && $_GET['status'] != 'all')): ?>
                                <a href="admin_customers.php" class="bg-gradient-to-r from-red-500 to-pink-600 text-white px-8 py-3 rounded-xl font-semibold hover:from-red-600 hover:to-pink-700 transition-all flex items-center gap-2">
                                    <i class="fas fa-times"></i>
                                    <span>Clear</span>
                                </a>
                            <?php endif; ?>
            ...
            <!-- Customer Statistics -->
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6 mb-8 md:mb-12 fade-in-up">
                <?php 
                $statCards = [
                    [
                        'icon' => 'fas fa-users', 
            ...
            <div class="glass-card rounded-2xl p-6 md:p-8 fade-in-up">
                <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
                    <h2 class="text-white text-xl md:text-2xl font-bold flex items-center">
                        ...
                        Customer List
                        <span class="ml-3 text-gray-400 text-sm">(<?php echo $total_rows; ?> customers found)</span>
                    </h2>
                    
                    <div class="flex items-center gap-3">
                        <a href="add_customer.php" class="bg-gradient-to-r from-green-500 to-emerald-600 text-white px-6 py-3 rounded-xl font-semibold hover:from-green-600 hover:to-emerald-700 transition-all flex items-center gap-2">
                ...
                <!-- Bulk Actions -->
                <form method="POST" action="" id="bulkForm" class="bulk-actions fade-in-up">
                    <div class="flex flex-col md:flex-row md:items-center gap-4 w-full">
                        <div class="flex items-center gap-3">
                            ...
                            <label for="selectAll" class="text-white text-sm font-medium">Select All</label>
                            <span id="selectedCount" class="text-gray-400 text-sm">0 selected</span>
                        </div>
                        
                        <div class="flex flex-1 gap-3">
                            <select name="bulk_action" class="form-input" style="min-width: 160px;">
                            ...
                            <button type="submit" class="bg-gradient-to-r from-blue-500 to-purple-600 text-white px-6 py-3 rounded-xl font-semibold hover:from-blue-600 hover:to-purple-700 transition-all">
                                Apply
                            </button>
                        </div>
                    </div>
                ...
                <div class="table-container">
                    <table class="table w-full">
                        <thead>
                            <tr>
                                <th width="50">
                                    <input type="checkbox" id="selectAllHeader" class="checkbox" onchange="toggleSelectAll()">
                                </th>
                                <th>Customer</th>
                                <th>Contact</th>
                                <th>Reports</th>
                            ...
                            <tr class="hover:bg-white/5 transition-colors">
                                <td>
                                    <input type="checkbox" name="selected_customers[]" 
                                           ...
                                           class="checkbox customer-checkbox" 
                                ...
                                <td>
                                    <div class="text-white mb-1"><?php echo htmlspecialchars($customer['customer_email']); ?></div>
                                    <div class="text-gray-400 text-sm"><?php echo htmlspecialchars($customer['phone']); ?></div>
                                </td>
                                <td>
                                    <div class="flex flex-col">
                                        <div class="text-white font-bold"><?php echo $customer['reports_count']; ?> total</div>
                                        <div class="text-green-400 text-sm"><?php echo $customer['completed_reports']; ?> completed</div>
                                    </div>
                                </td>
                                <td>
        ...
        <footer class="bg-gray-900/50 backdrop-blur-lg text-white py-8 md:py-12 mt-12 safe-area-bottom">
            <div class="container mx-auto px-4">
                <div class="flex flex-col md:flex-row justify-between items-start">
                    <div class="mb-8 md:mb-0 md:w-1/3">
                        ...
                        <p class="text-gray-400 text-sm md:text-base">
                            Customer management portal for monitoring user contributions and engagement.
                        </p>
                    </div>
                    
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-8 md:gap-16 w-full md:w-2/3">
                        <div>
                            <h4 class="text-lg font-bold mb-4 text-blue-400">Customer Management</h4>
                            <ul class="space-y-2">
                                <li><a href="admin_customers.php" class="text-gray-400 hover:text-white transition-colors text-sm md:text-base">Customer List</a></li>
                                <li><a href="add_customer.php" class="text-gray-400 hover:text-white transition-colors text-sm md:text-base">Add New Customer</a></li>
                                <li><a href="admin_reports.php" class="text-gray-400 hover:text-white transition-colors text-sm md:text-base">Customer Reports</a></li>
                                <li><a href="admin_analytics.php" class="text-gray-400 hover:text-white transition-colors text-sm md:text-base">Customer Analytics</a></li>
                            </ul>
                        </div>
                        <div>
                            ...
                            <ul class="space-y-2">
                                <li class="text-gray-400 text-sm md:text-base flex items-start">
                                    <i class="fas fa-users mr-2 text-sm mt-1 text-blue-400"></i> 
                                    <?php echo $stats['total_customers']; ?> Total Customers
                                </li>
                                <li class="text-gray-400 text-sm md:text-base flex items-start">
                                    <i class="fas fa-user-check mr-2 text-sm mt-1 text-green-400"></i>
                                    <?php echo $stats['active_customers']; ?> Active Customers
                                </li>
                                <li class="text-gray-400 text-sm md:text-base flex items-start">
                                    <i class="fas fa-trophy mr-2 text-sm mt-1 text-yellow-400"></i>
                                    <?php echo $stats['total_points']; ?> Total Points
                                </li>
                            </ul>
                        </div>
                    </div>
                ...
                <div class="border-t border-gray-800 mt-8 pt-8 text-center text-gray-400 text-sm md:text-base">
                    <p>&copy; 2024 WasteWise Customer Management. Showing page <?php echo $page; ?> of <?php echo $total_pages; ?>.</p>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            navItems.forEach(item => {
                ...
                if (href && href.includes(currentPage) || 
                    (currentPage === '' && href.includes('customers'))) {
            ...
            if (bulkForm) {
                bulkForm.addEventListener('submit', function(e) {
                    ...
                    if (selectedCount === 0) {
                        e.preventDefault();
                        alert('Please select at least one customer to perform bulk action.');
                        return false;
                    }
                    ...
                    if (bulkAction === 'delete') {
                        if (!confirm(`Are you sure you want to delete ${selectedCount} customer(s)? This action cannot be undone.`)) {
                            e.preventDefault();
                            return false;
                        }
                    }
                    ...
                    if (bulkAction === 'deactivate') {
                        if (!confirm(`Are you sure you want to deactivate ${selectedCount} customer(s)?`)) {
                            e.preventDefault();
                            return false;
                        }
                    }
